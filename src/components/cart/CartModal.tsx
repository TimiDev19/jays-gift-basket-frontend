...
const CartModal = () => {
  ...
  // Per-basket custom notes: keyed by cart item id -> array of notes,
  // one entry per unit of that basket (so qty 2 = 2 separate note boxes).
  ...
  // Keep itemNotes in sync with the cart: add note slots for new items/units,
  // drop slots for removed items/units, preserve what's already been typed.
  useEffect(() => {
    setItemNotes((prev) => {
      const next: Record<string, string[]> = {};
      let changed = false;

      for (const item of cart as CartItemType[]) {
        const key = String(item.id);
        const existing = prev[key] ?? [];
        const notesForItem = Array.from(
          { length: item.quantity },
          (_, i) => existing[i] ?? ""
        );
        next[key] = notesForItem;

        if (
          existing.length !== notesForItem.length ||
          existing.some((val, i) => val !== notesForItem[i])
        ) {
          changed = true;
        }
      }
  ...
  const buildTemplateParams = () => {
    const orderSummary = cart
      .map((item: CartItemType) => {
        const lineTotal = `${item.name} x${item.quantity} - ₦${(
          item.price * item.quantity
        ).toFixed(2)}`;

        const notes = itemNotes[String(item.id)] ?? [];
        const noteLines = notes
          .map((note, i) => {
            const trimmed = note.trim();
            if (!trimmed) return null;
            return item.quantity > 1
              ? `    📝 Note (Basket ${i + 1} of ${item.quantity}): ${trimmed}`
              : `    📝 Note: ${trimmed}`;
          })
          .filter(Boolean)
          .join("\n");

        return noteLines ? `${lineTotal}\n${noteLines}` : lineTotal;
      })
      .join("\n");

    const hasAnyNotes = Object.values(itemNotes).some((notes) =>
    ...
    return {
      ...
      preferred_delivery_time:
        fulfillmentType === "delivery" && preferredDeliveryTime
          ...
          : "N/A",
      order_summary: orderSummary,
      has_basket_notes: hasAnyNotes ? "Yes" : "No",
      delivery_fee: `₦${deliveryFee.toFixed(2)}${
        deliveryMultiplier > 1 ? ` (${deliveryMultiplier}× surcharge)` : ""
      }`,
      total_amount: totalCost.toFixed(2),
  ...
  return (
    <div className="bg-white px-8 py-8 rounded-xl flex flex-col gap-4 max-h-[90dvh] w-full overflow-y-auto">
      ...
      <div className="border-b border-[#A87F3D] pb-4 flex flex-col gap-4">
        {cart.length === 0 ? (
          <p className="mt-4 font-semibold italic">No Items In Cart 🤧</p>
        ) : (
          cart.map((crt: CartItemType, index: number) => (
            <div key={index} className="flex flex-col gap-2">
              ...
              {/* Optional custom note(s) for this basket — one box per unit */}
              <div className="flex flex-col gap-2 pl-2 border-l-2 border-[#A87F3D]/20">
                {Array.from({ length: crt.quantity }).map((_, unitIndex) => (
                  <div key={unitIndex} className="flex flex-col gap-1">
                    <label className="text-[0.7rem] text-gray-500 font-medium uppercase tracking-wide">
                      {crt.quantity > 1
                        ? `Note for basket ${unitIndex + 1} of ${crt.quantity} (optional)`
                        : "Add a custom note (optional)"}
                    </label>
                    <textarea
                      placeholder="e.g. Happy Birthday, Mum! Love you ❤️"
                      value={itemNotes[String(crt.id)]?.[unitIndex] ?? ""}
                      onChange={(e) =>
                        updateItemNote(crt.id, unitIndex, e.target.value)
                      }
                      maxLength={NOTE_MAX_LENGTH}
                      className="border border-gray-300 p-2 rounded outline-none text-[0.85rem] min-h-[60px] bg-white"
                    />
                  </div>
                ))}
              </div>
